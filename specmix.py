...
def specmix(x, y, alpha, prob, f_band_size, t_band_size):
    ...
    if k > 1 - prob:
        ...
        mask_start = random.randint(0, x.size()[2] - f_band_size)        
        mask_end = mask_start + f_band_size
        x[:, mask_start:mask_end, :] = x[batch_idx, mask_start:mask_end,: ]
        lam_freq = (f_band_size / x.size()[2])
        mask_start = random.randint(0, x.size()[1] - t_band_size)
        mask_end = mask_start + t_band_size
        x[:, :, mask_start:mask_end] = x[batch_idx, :, mask_start:mask_end]
        lam_time = ((mask_end - mask_start) / x.size()[1])
